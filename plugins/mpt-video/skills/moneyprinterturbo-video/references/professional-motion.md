...
For evidence-led finance or company explainers, first complete
`evidence-motion-workflow.md` and `scene-contract.md`. This file governs visual
direction; those files govern facts, geometry, and executable QA.
...
- Use generated footage only as a plate. Render exact text, Korean captions,
  numbers, charts, maps, and brand marks in deterministic layers.
- Give each scene purposeful continuous motion tied to the argument. Do not add
  glowing objects or unrelated machinery merely to keep pixels moving.
...
- For a declared scene contract, run `validate_scene_manifest.py` before render
  and `probe_scene_frames.py` after final subtitle assembly.
