...
$scenePath = Join-Path $motionDirectory "mcd_short_v4.py"

uv run --project $motionDirectory manim "-q$Quality" --format mp4 --media_dir $MediaDirectory $scenePath MCDDividendYieldShort
if ($LASTEXITCODE -ne 0) {
    throw "Manim render failed with exit code $LASTEXITCODE"
}

$rendered = Get-ChildItem -LiteralPath $MediaDirectory -Recurse -Filter "MCDDividendYieldShort.mp4" |
